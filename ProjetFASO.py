...
def action(temps,nbrOption=0):
    ...
    while (tBoucle < tStart+temps) and (mvtPosition == 0) and not bouton1 and not bouton2 :
        # Potentiomètre
        ...
        if posCourante-posStart > 256//nbrOption :
            ...
            mvtPosition=-1
        # Bouton 1
        if grovepi.digital.read(3)==1:
            bouton1=True
        # Bouton 2
        if grovepi.digital.read(2)==1:
            bouton2=True
...
#"""
    ...
    while not a: # On évalue action mais pas la première fois -> Do...While
        i=0 # Itérateur de ligne
        while i<nbrLigne and not action(2):
